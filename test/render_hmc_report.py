...
def save_tune_summary(summary: dict, out_dir: Path) -> None:
    ...
    for case in summary["cases"]:
        rows = case["rows"]
        rows = sorted(rows, key=lambda row: (row["hmc_mass"], row["nfrog"] * row["hmc_dt"]))
        ...
        labels = [f"{int(row['nfrog'])}x{row['hmc_dt']:g}\nm={row['hmc_mass']:g}" for row in rows]
...
def write_stage_markdown(summary: dict, case_rows: list[dict[str, str]], trace_files: list[str], out_dir: Path) -> None:
    lines = [
        ...
        "",
        "- `stable_window`: gate-observable drift is small compared with the observed span",
        "- `slow_drift`: visible drift remains but the run is no longer obviously stuck",
        "- `strong_drift`: gate observables still drift strongly across the retained window",
        "- `stuck_or_invalid`: acceptance or ESS indicates that at least one repeat is effectively unusable",
        "",
        "| Case | Acceptance | tau_int(doubleOcc) | ESS/sec | squareOcc drift/span | IPR drift/span | Status |",
        "| --- | ---: | ---: | ---: | ---: | ---: | --- |",
    ]
    for row in case_rows:
        lines.append(
            "| {name} | {accept:.3f} | {tau:.3f} | {ess:.3f} | {sq:.3f} | {ipr:.3f} | {status} |".format(
                name=row["name"],
