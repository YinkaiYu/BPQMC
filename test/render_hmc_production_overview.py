...
def write_report(
    ...
    lines = [
        ...
        "",
        "| case | representative stage | beta | dtau | repeats | bins | cut | warm | samples/post | max drift | repeat span | nfrog | dt | mass | m_uniform | m_shell1 | acceptance | tau_int(doubleOcc) | ESS/sec | status |",
        "| --- | --- | ---: | ---: | --- | ---: | ---: | ---: | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | --- |",
    ]
    ...
    lines.extend(
        ...
        "## Stage Summary",
        "",
        "| label | case | beta | dtau | repeats | bins | cut | warm | samples/post | max drift | repeat span | nfrog | dt | mass | m_uniform | m_shell1 | acceptance | tau_int(doubleOcc) | ESS/sec | status |",
        "| --- | --- | ---: | ---: | --- | ---: | ---: | ---: | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | --- |",
        ]
    )
    ...
    lines.extend(
        [
            ...
            "",
            "| label | case | beta | dtau | viable | nfrog | dt | mass | m_uniform | m_shell1 | acceptance | tau_int(doubleOcc) | ESS/sec | DeltaH_abs_max |",
            "| --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |",
        ]
    )
    for row in sorted(tune_recommended, key=lambda item: (str(item["case"]), float(item["Nbos"]) if "Nbos" in item else 0.0, float(item["U2"]) if "U2" in item else 0.0, float(item["beta"]), str(item["label"]))):
        ...
        row["label_link"] = linked_label(str(row["work_root"]), str(row["label"]), output_dir)
        lines.append(
            "| {label_link} | {case} | {beta:.6g} | {dtau:.6g} | {recommended_viable} | {nfrog} | {hmc_dt:.6g} | {hmc_mass:.6g} | {hmc_mass_spatial_uniform:.6g} | {hmc_mass_spatial_shell1:.6g} | {acceptance_mean:.3f} | {tau_int_doubleOcc_mean:.3f} | {ess_per_sec_doubleOcc_mean:.3f} | {hmc_deltaH_abs_max:.3f} |".format(
                **row
            )
        )
    if live_reports:
    ...
    for case in sorted({str(row["case"]) for row in stage_cases}):
        ...
        lines.extend(
            [
                ...
                f"## Case Summary: {case}",
                "",
                "| label | beta | dtau | repeats | bins | cut | warm | samples/post | max drift | repeat span | nfrog | dt | mass | m_uniform | m_shell1 | acceptance | tau_int(doubleOcc) | ESS/sec | status |",
                "| --- | ---: | ---: | --- | ---: | ---: | ---: | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | --- |",
            ]
        )
...
def main() -> int:
    ...
    write_csv(
        ...
        tune_candidates,
        ["label", "work_root", "case", "beta", "dtau", "nfrog", "hmc_dt", "hmc_mass", "hmc_mass_spatial_uniform", "hmc_mass_spatial_shell1", "acceptance_mean", "tau_int_doubleOcc_mean", "ess_per_sec_doubleOcc_mean", "hmc_deltaH_abs_max"],
    )
    write_csv(
        output_dir / "tune_recommended.csv",
        tune_recommended,
        ["label", "work_root", "case", "beta", "dtau", "recommended_viable", "nfrog", "hmc_dt", "hmc_mass", "hmc_mass_spatial_uniform", "hmc_mass_spatial_shell1", "acceptance_mean", "tau_int_doubleOcc_mean", "ess_per_sec_doubleOcc_mean", "hmc_deltaH_abs_max"],
    )
